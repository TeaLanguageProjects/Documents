...
如果您发现了本文档的错误，欢迎您发起Issues指出问题所在。
...
| 0x6B      | ISHL               | (Byte)Data | 将本地栈顶的int按指定位数左移，并将结果推送到本地栈上          |
| 0x6C      | ISHR               | (Byte)Data | 将本地栈顶的int按指定位数右移，并将结果推送到本地栈上          |
| 0x6D      | IUSHR              | (Byte)Data | 将本地栈顶的int按指定位数无符号右移，并将结果推送到本地栈上    |
| 0x6E      | LSHL               | (Byte)Data | 将本地栈顶的long按指定位数左移，并将结果推送到本地栈上         |
| 0x6F      | LSHR               | (Byte)Data | 将本地栈顶的long按指定位数右移，并将结果推送到本地栈上         |
| 0x70      | LUSHR              | (Byte)Data | 将本地栈顶的long按指定位数无符号右移，并将结果推送到本地栈上   |
...
| 0xB5        | VM_ISHL          | (Byte)Data      | 将虚拟机栈顶的int值左移指定位数，并将结果压入虚拟机栈             |
| 0xB6        | VM_ISHR          | (Byte)Data      | 将虚拟机栈顶的int值右移指定位数，并将结果压入虚拟机栈             |
| 0xB7        | VM_IUSHR         | (Byte)Data      | 将虚拟机栈顶的int值无符号右移指定位数，并将结果压入虚拟机栈         |
| 0xB8        | VM_LSHL          | (Byte)Data      | 将虚拟机栈顶的long值左移指定位数，并将结果压入虚拟机栈            |
| 0xB9        | VM_LSHR          | (Byte)Data      | 将虚拟机栈顶的long值右移指定位数，并将结果压入虚拟机栈            |
| 0xBA        | VM_LUSHR         | (Byte)Data      | 将虚拟机栈顶的long值无符号右移指定位数，并将结果压入虚拟机栈        |
